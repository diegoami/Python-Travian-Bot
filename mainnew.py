...
def is_in_corso():
    return travian_party.festa_in_corso() or not (travian_risorse.has_municipio())

def will_raid():
    return ((loopcounter % travian_account.frequency == 0) or (travian_schiva.devo_spostare(current_villo)))

def check_festa():    
    cfesta = is_in_corso()  or (not(travian_mail.get_string("FESTA") == 1)  or (travian_schiva.devo_spostare(current_villo)) ) 
